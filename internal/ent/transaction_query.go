...
func (_q *TransactionQuery) QueryInvoice() *InvoiceQuery {
...
func (_q *TransactionQuery) WithInvoice(opts ...func(*InvoiceQuery)) *TransactionQuery {
	query := (&InvoiceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if _q.withUser != nil || _q.withInvoice != nil || _q.withCategory != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *TransactionQuery) loadInvoice(ctx context.Context, query *InvoiceQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Invoice)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Transaction)
	for i := range nodes {
